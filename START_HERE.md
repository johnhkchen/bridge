# START HERE — Bridge (overseer brief)

You are the **overseer** of a `vend → lisa` build of **Bridge**: real single-player Contract
Bridge, offline-first, shipped as one self-contained HTML file. This repo is the *tortoise* in
b28.dev's tortoise-vs-hare series — its whole point is that a reviewed, tested pipeline beats a
one-shot on everything except day-one speed. Your job is not to write the game. It is to **drive
the board one pull at a time, protect the invariants below, and make the fork calls** vend puts
in front of you.
...
**Definition of done:** *a curious beginner opens it a second time — and understands a bid they
didn't before.* Fun and learning per unit of effort, not feature count.
...
- **One self-contained offline `dist/index.html`.** Built with Vite + `vite-plugin-singlefile`
  (JS + CSS + inline-SVG card art inlined). The single file is a **compile target, not the
  authoring format.** No server, no SSR, no router, no code-splitting, no multi-page output.
- **`src/core/` is pure.** Engine + AI + PBN notation, **zero DOM/framework imports**,
  property-tested with vitest. It **never imports `src/app/`.** This is where correctness lives;
  it is big in *tests*, not runtime.
...
- **Event-sourced.** A deal *is* `seed + auction + play`; board state is always **derived by
  folding the pure engine.** Undo, replay, and post-hand review are folds over prefixes — never a
  mutable board.
- **PBN is the public contract.** The engine's interface is *notation in → legal calls / legal
  plays / next state out.* Tests are PBN round-trips; a bug report is a PBN deal.
- **Seeded RNG.** Full deals must be deterministically simulatable — AI-vs-AI self-play doubles
  as attract mode.
- **AI plays fair.** The bidding AI (`record → call`) and play AI (`record → card`) are stateless
  peripherals that reason only from what their seat legitimately knows — no peeking at hidden
  hands.
...
Then pull **Tier 1 — a finishable deal** (deal → legal auction → legal 13-trick play → correct
scoring, with one competent bot table). Teaching (Tier 2) is the crown, but a legal, finishable
deal comes first — without it there is nothing.
...
Foundational work (the engine types, the PBN contract, shared modules) should be early tickets
that others depend on; independent features touching different files run in parallel.

---

## When in doubt

Pull the signal that makes a beginner's next session more fun or more instructive. Fund small,
sweep often, and read the review output — the pipeline's value is that being wrong costs a
sentence (`vend annotate`) instead of a rewrite.
